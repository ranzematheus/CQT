...
OpenLayers.Layer.GeoRSS = OpenLayers.Class(OpenLayers.Layer.Markers, {
    ...
    */
    ...
    parseData: function(ajaxRequest) {
		...
		var boxList =document.getElementById("listMarkers");
		var optionsSelect = "";
        for (var i=0, len=features.length; i<len; i++) {
			...
			//optionsSelect += "<option value="+ title + ">"+ title +"</option>";
			...
			marker.events.register('touchstart', feature, this.markerClick);
            //this.addMarker(marker);
        }
		boxList.innerHTML = optionsSelect;
    ...
    markerClick: function(evt) {
        ...
        if (!sameMarkerClicked) {
			...
			OpenLayers.Event.observe(popup.div, "touchstart",
                OpenLayers.Function.bind(function() { 
                    for(var i=0, len=this.layer.map.popups.length; i<len; i++) { 
                        this.layer.map.removePopup(this.layer.map.popups[i]); 
                    }
