 ...
 OpenLayers.Control.PanZoomBar = OpenLayers.Class(OpenLayers.Control.PanZoom, {
    ...
    */
    ...
    draw: function(px) {
		...
		//alert($(document).height() + "  " + $("#map").height());
		//alert($(document).width() + "  " + $("#map").width());
		
		px.x = $("#resetZoom").width() + $("#aboutBox").width() + $("#hideMarkers").width()+ 12;
		px.y = $("#map").height() - $("#navBar").height() + 7;
        ...
        if (!this.panIcons) {
			...
			var centered = new OpenLayers.Pixel(5, px.y + sz.h/2);
					//...... centered -> x = 4 e y = 13

            this._addButton("zoomout", "btMenos.png", centered.add(sz.w*3+5,0), sz);
            centered = this._addZoomBar(centered.add(sz.w * 4 + 5,0));
            this._addButton("zoomin", "btMais.png", centered, sz);
			
			
        }
        else {

            this._addButton("zoomout", "btMenos.png", px, sz);
            centered = this._addZoomBar(px.add(sz.w,0));
            this._addButton("zoomin", "btMais.png", centered, sz);
        }
    ...
    */
    ...
    _addZoomBar:function(centered) {
		...
		var zoomsToEnd;
		if ( this.map.getZoom() == 0 && primeiraVez ) {
			zoomsToEnd= this.map.getNumZoomLevels() - this.map.getZoom() - 1;
		} else {
			zoomsToEnd = this.map.getZoom();
		}
		//alert(this.map.getZoom() + " " + zoomsToEnd);
...
//			if (evt.xy.x > (-(parseInt(this.zoombarDiv.style.width))/5)  &&  evt.xy.x < ((parseInt(this.zoombarDiv.style.width))/2)) {
    ...
    */
    moveZoomBar:function() {
        ...
        this.slider.style.left = (newLeft-1) + "px";
